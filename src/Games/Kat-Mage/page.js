...
const topicQuestions = {
  ...
  'data-types': {
    narrative: 'The spikes below are made of different data types.\nOnly understanding them will save you.',
    instruction: 'Answer the question to desummon the spikes.',
    question: 'Question: Write code to check the data type of the value 3.14'
  },
  'type-casting': {
    narrative: 'The spikes need a type conversion to disappear.\nCast them away with your knowledge!',
    instruction: 'Answer the question to desummon the spikes.',
    question: 'Question: Convert the string "42" to an integer and store it in a variable'
  },
  'user-input': {
    narrative: 'The spikes want user input to vanish.\nTake input and set yourself free!',
    instruction: 'Answer the question to desummon the spikes.',
    question: 'Question: Write code to take a number input from user and print its square'
  }
};
...
export default function Game({ topic }) {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('openEditor', trueSetter); };
  }, []);
    ...
    async function initGame() {
      ...
      class PreloadScene extends Phaser.Scene {
        ...
        preload() {
          ...
          this.load.spritesheet('white-cat-die', '/assets/Meow-Knight_Death.png', { frameWidth: 16, frameHeight: 24, endFrame: 11 });
          this.load.spritesheet('white-cat', '/assets/Meow-Knight_Idle.png', { frameWidth: 16, frameHeight: 24, endFrame: 5 });
          this.load.spritesheet('white-cat-jump', '/assets/Meow-Knight_Jump.png', { frameWidth: 16, frameHeight: 24, endFrame: 14 });
        }

        create() {
          this.anims.create({ key: 'die', frames: this.anims.generateFrameNumbers('white-cat-die', { start: 0, end: 5 }), frameRate: 8 });
          this.anims.create({ key: 'idle', frames: this.anims.generateFrameNumbers('white-cat', { start: 0, end: 5 }), frameRate: 8, repeat: -1 });
          this.anims.create({ key: 'jump', frames: this.anims.generateFrameNumbers('white-cat-jump', { start: 0, end: 5 }), frameRate: 8 });
      ...
      class BaseLevel extends Phaser.Scene {
        ...
        levelCompleteSequence(nextSceneKey) {
          ...
          const successText = this.add.text(450, 450, 'MISSION COMPLETE', {
            fontSize: '56px',
      ...
      class GameScene extends BaseLevel {
        ...
        create() {
          ...
          this.title = this.add.image(430, 490, 'title').setDisplaySize(600, 400);
                ...
                onComplete: () => {
                  ...
                  this.time.delayedCall(600, () => {
                   this.pauseScreenGroup = this.add.group();
                    ...
                    // Narrative Text
                    const narrativeText = this.add.text(450, 320, qData.narrative, { 
                      fontSize: '28px', 
                      fontFamily: 'Arial, sans-serif',
                      color: '#e2e8f0',
                      align: 'center',
                    ...
                    // Instruction Text
                    const instructionText = this.add.text(450, 430, qData.instruction, { 
                      fontSize: '22px', 
                      fontFamily: 'Arial, sans-serif',
                      color: '#fbbf24',
                      align: 'center',
                    ...
                    const questionText = this.add.text(450, 560, qData.question, { 
                      fontSize: '24px', 
        ...
        fallMotion() {
          this.tweens.add({
            targets: this.cat,
            x: 600, y: 600, duration: 2000, ease: 'Power4',
            onComplete: () => {
              ...
              this.add.text(450, 400, 'YOU FAILED', { 
                fontSize: '56px', 
                fontFamily: 'Arial, sans-serif',
