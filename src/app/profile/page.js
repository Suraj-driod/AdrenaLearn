'use client'
import { Edit3, BookOpen, Gamepad2, Calendar, Clock, Award, Rocket, Trophy, Zap, Target, Flame, Sword, Medal, Wind, Cat, BarChart3 } from 'lucide-react'
import Sidebar from '../components/Sidebar'

const badges = [
  { name: 'First Lesson', icon: <BookOpen className="w-7 h-7 text-[#f04e7c]" />, desc: 'Completed your first lesson', earned: true },
  { name: '3 Day Streak', icon: <Flame className="w-7 h-7 text-[#fbc13a]" />, desc: 'Maintained a 3 day streak', earned: true },
  { name: 'Perfect Score', icon: <Target className="w-7 h-7 text-[#7c3aed]" />, desc: 'Got 100% in a game round', earned: true },
  { name: 'Top 10', icon: <Trophy className="w-7 h-7 text-[#fbc13a]" />, desc: 'Reached Top 10', earned: true },
  { name: 'Speed Demon', icon: <Zap className="w-7 h-7 text-[#f04e7c]" />, desc: 'Finished under 1 min', earned: false },
  { name: 'Week Warrior', icon: <Sword className="w-7 h-7 text-[#1e7a4e]" />, desc: '7 day streak achieved', earned: false },
]

const gamePerformance = [
  { name: 'Spaceship', icon: <Rocket className="w-6 h-6 text-[#f04e7c]" />, avgScore: 1180, games: 12 },
  { name: 'Subway', icon: <Wind className="w-6 h-6 text-[#fbc13a]" />, avgScore: 950, games: 8 },
  { name: 'Balloon', icon: <Target className="w-6 h-6 text-[#7c3aed]" />, avgScore: 1050, games: 10 },
  { name: 'Cat Rescue', icon: <Cat className="w-6 h-6 text-[#1e7a4e]" />, avgScore: 890, games: 6 },
]

const recentActivity = [
  { action: 'Completed Lesson 7: Lists in Python', time: '2 hours ago', icon: <BookOpen className="w-5 h-5" />, color: 'bg-[#d8ecff]' },
  { action: 'Played Spaceship Mission — 1,250 pts', time: '3 hours ago', icon: <Rocket className="w-5 h-5" />, color: 'bg-[#ffd6e4]' },
  { action: 'Earned badge: Perfect Score', time: '1 day ago', icon: <Medal className="w-5 h-5" />, color: 'bg-[#fff3c4]' },
  { action: 'Played Balloon Shooter — 980 pts', time: '2 days ago', icon: <Target className="w-5 h-5" />, color: 'bg-[#d4f0e0]' },
  { action: 'Completed Lesson 6: Loops', time: '3 days ago', icon: <BookOpen className="w-5 h-5" />, color: 'bg-[#d8ecff]' },
]
      ...
      <main className="lg:ml-64 pt-16 lg:pt-0 min-h-screen">
        <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
            {[
              { label: 'Total XP', value: '2,450', icon: <Zap className="w-6 h-6 text-[#f04e7c]" />, color: 'bg-[#ffd6e4]' },
              { label: 'Global Rank', value: '#42', icon: <Trophy className="w-6 h-6 text-[#92600e]" />, color: 'bg-[#fff3c4]' },
              { label: 'Lessons Done', value: '7', icon: <BookOpen className="w-6 h-6 text-[#2563eb]" />, color: 'bg-[#d8ecff]' },
              { label: 'Avg Accuracy', value: '78%', icon: <Target className="w-6 h-6 text-[#1e7a4e]" />, color: 'bg-[#d4f0e0]' },
            ].map((stat, i) => (
              <div key={i} className={`${stat.color} p-5 rounded-3xl border-2 border-[#1e1b26] shadow-[4px_4px_0px_#1e1b26] hover:shadow-[6px_6px_0px_#1e1b26] hover:-translate-y-1 hover:-translate-x-1 transition-all group`}>
                <div className={`w-12 h-12 rounded-2xl bg-white border-2 border-[#1e1b26] flex items-center justify-center mb-3`}>{stat.icon}</div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-[#ede4ff] p-6 rounded-[32px] border-2 border-[#1e1b26] shadow-[4px_4px_0px_#1e1b26]">
                <h3 className="font-[Outfit] font-black text-xl mb-5 flex items-center gap-2 text-[#1e1b26]"><Medal className="w-5 h-5 text-[#fbc13a]" /> Badges</h3>
              ...
              <div className="bg-[#e4f1ff] p-6 rounded-[32px] border-2 border-[#1e1b26] shadow-[4px_4px_0px_#1e1b26]">
                <h3 className="font-[Outfit] font-black text-xl mb-5 flex items-center gap-2 text-[#1e1b26]"><Gamepad2 className="w-5 h-5 text-[#f04e7c]" /> Game Performance</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                  {gamePerformance.map((game, i) => (
                    <div key={i} className="bg-white border-2 border-[#1e1b26] rounded-2xl p-4 text-center hover:shadow-[3px_3px_0px_#1e1b26] hover:-translate-y-0.5 hover:-translate-x-0.5 transition-all group">
                      <div className="mb-2 group-hover:scale-110 transition-transform flex justify-center">{game.icon}</div>
              ...
              <div className="bg-[#fff8e7] p-6 rounded-[32px] border-2 border-[#1e1b26] shadow-[4px_4px_0px_#1e1b26]">
                <h3 className="font-[Outfit] font-black text-xl mb-6 flex items-center gap-2 text-[#1e1b26]"><BarChart3 className="w-5 h-5 text-[#f04e7c]" /> Weekly XP</h3>
            ...
            <div>
              <div className="bg-[#d4f0e0] p-6 rounded-[32px] border-2 border-[#1e1b26] shadow-[4px_4px_0px_#1e1b26] lg:sticky lg:top-6">
                <h3 className="font-[Outfit] font-black text-xl mb-5 flex items-center gap-2 text-[#1e1b26]"><Clock className="w-5 h-5 text-[#1e7a4e]" /> Recent Activity</h3>
                <div className="space-y-4">
                  {recentActivity.map((activity, i) => (
                    <div key={i} className="flex items-start gap-3 group bg-white/60 p-3 rounded-2xl border-2 border-transparent hover:border-[#1e1b26] hover:bg-white transition-all">
                      <div className={`w-10 h-10 rounded-xl border-2 border-[#1e1b26] ${activity.color} flex items-center justify-center shrink-0`}>{activity.icon}</div>
